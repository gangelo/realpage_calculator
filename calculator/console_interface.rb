...
module RealPage
  module Calculator
    # Provides a console interface that obtains user input from $stdin to be
    # processed by a CalculatorService or derived class object and returned to
    ...
    class ConsoleInterface < IOInterface
      ...
      # Starts the process of receiving input from $stdin and sets the interface
      # state to the #opened_state.
      def accept
        super
      ...
      #
      # @return [String] Returns the input received with new line characters
      # removed.
      def receive
      ...
      # @calculator_service as an observer. When @calculator_service input is
      # received, it should be subsequently passed to the interface output
      # stream.
      #
      # @param [CalculatorResult] calculator_result A CalculatorResult object
      # that contains the CalculatorService
      ...
      def receive_calculator_result_error(calculator_result)
        ...
        # Interpolate the error label and error message so that it's formatted
        # nicely for display, in the user's locale.
      ...
      #
      # @param [TrueClass, FalseClass] new_line If new_line is true, a new line
      # character will be added before the prompt is displayed; the prompt will
      # be displayed on the same line otherwise.
